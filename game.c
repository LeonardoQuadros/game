...
void draw_map()
{
    ...
    for(i=0; i<120; i+=3)
    {
        ...
        {
            if(map[i][j] == '|')
                al_draw_scaled_bitmap(wall, 0, 0, 19, 16, 10*j, 10*i+5, 30, 30, 0);
            if(map[i][j] == 'R')
                al_draw_scaled_bitmap(stone, 0, 0, 32, 32, 10*j, 10*i+5, 30, 30, 0);
            if(map[i][j] == 'O')
                al_draw_scaled_bitmap(wall, 0, 0, 19, 16, 10*j, 10*i+5, 30, 30, 0);
            if(map[i][j] == 'X')
                al_draw_scaled_bitmap(position[player.pos], 0, 0, 18, 29, 10*j, 10*i+5, 30, 30, 0);
        }
    }
}
...
void player_movement()
{
    ...
    {
        player.pos = 3;
        if(map[player.y][player.x-3] == '*' || map[player.y][player.x-3] == ' ')
        {
    ...
    {
        player.pos = 0;
        if(map[player.y-3][player.x] == '*' || map[player.y-3][player.x] == ' ')
        {
            for(i=player.y;i<player.y+3;i++)
                 for(j=player.x;j<player.x+3;j++)
                 {
...
int main()
{
    ...
    buffer = al_create_bitmap(1500, 1205);
    ...
    int i, j, aux1, aux2, scorelist[10];
    FILE *arq;
